...
import com.github.overview_repo.domain.Address;
import com.github.overview_repo.domain.SupplyPoint;
import com.github.overview_repo.domain.SupplyPointFilter;
...
public class SupplyPointMapper extends DynamicEntityMapper<SupplyPoint, SupplyPointFilter> {
	...
	public List<Condition> composeFilterConditions(SupplyPointFilter filter) {
		...
		if (filter.getCustomerId() != null) {
			conditions.add(Condition.eq(customer_id, filter.getCustomerId()));
		}
		if (filter.getCustomerIds() != null) {
			conditions.add(Condition.in(customer_id, CollectionFuns.toObjectList(filter.getCustomerIds())));
		}
		return conditions;
	}
